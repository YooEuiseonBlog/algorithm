...
public class DayOfYear {
    ...
    //서기 y년 m월 d일의 그 해 경과 일 수를 구함
    public static int dayOfYearV2(int y, int m, int d) {
        int rest = d;
        Map<Integer, Integer> map = new HashMap<>();
        while (m>1) {
            d += mdays[isLeap(y)][m-2];
            map.put(m, mdays[isLeap(y)][m-2]);
            m--;
        }
        if (m==1) {
            map.put(m, rest);
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            System.out.printf("그 해 %d일째입니다.\n", dayOfYearV2(year, month, day));
            System.out.printf("그 해 남은 %d일입니다.\n", Q09.leftDayOfYear(year, month, day));
